...
async def agent_node():
    ...
    await loop.run_in_executor(None, slow_sync_call)
    print("Agent done")
...
2. Immediately `await`s a future that resolves when the thread finishes
3. Event loop is free to run other coroutines while the thread works
4. When the thread completes, your coroutine resumes with the result
...
2. Blocking the event loop is silent — it won't crash, it just destroys concurrency
3. `run_in_executor` is the bridge between async code and synchronous blocking libraries
4. Always ask: *"What is the event loop actually doing at this line?"*
