...
| Pattern | When to Use | Key Insight |
|---------|-------------|-------------|
| Fading-scaffold AI-tutor protocol | Learning an unfamiliar codebase or topic with AI without becoming dependent on it | Structure the session as a quiz/guide and fade support to zero on purpose: Q1 fully interrogated (why that? how did you find it?), next few with checks, the rest solo, AI reviews only at the end. Gradual release of responsibility, self-administered. The fade is the load-bearing part — support that never fades is a prosthetic; support that fades to zero is a teacher. Why THIS over open-ended AI Q&A: Q&A optimizes for answers, this optimizes for *being able to defend yourself* with the AI gone. (Invented mid-quiz 07-28, day 2 of the Independence Protocol.) |
| Constrain the search space, keep the finding | Huge unfamiliar codebase, don't know where to look | Ask AI to name candidate *files*, then locate the actual answer in the code yourself. Narrows tens of thousands of lines to a searchable set without outsourcing the understanding. Why THIS over "AI, explain this code": the explanation rep is the learning — giving it away buys speed today at the cost of independence tomorrow. |
| Frustration → decompose smaller | Mid-struggle rage, freeze, "I can't figure this out" | Frustration is data about the material, not a verdict on you: it means the chunk isn't broken down small enough yet. The move that un-froze day 1 (07-27): rebuild the task as smaller evidence-required blanks. "I can't" → "I haven't broken it down small enough yet." |
