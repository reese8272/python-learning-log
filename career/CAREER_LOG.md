...
| Prompt engineering | Building | Evaluate-before-delegate principle understood. + Prompt anatomy owned (2026-07-08): system is a top-level param (not messages[0]) → render order tools→system→messages makes it a cacheable prefix; assistant turns = re-sent prior outputs = the "memory"; max_tokens soft-truncate (stop_reason="max_tokens") vs context hard-reject (model_context_window_exceeded). Current pattern: assistant-prefill removed on 4.6+ (400) → Structured Outputs (output_config.format) *enforce* shape vs prefill *nudge*. Built a working extraction-call assembler. Level stays Building until defended cold in /sharpen. | 2026-07-08 | - |
