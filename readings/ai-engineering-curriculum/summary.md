...
- [x] **Anatomy of a prompt** (2026-07-08) — system/user/assistant turns, prompt composition, `max_tokens`. `T1` · src: Eden Marco lec 68, Anthropic Ch.1 *(taught: system is a top-level param not messages[0] & why (render order tools→system→messages = cacheable prefix); assistant turns = re-sent prior outputs = the whole "memory"; max_tokens SOFT-truncates (stop_reason="max_tokens") vs context window HARD-rejects (model_context_window_exceeded) → always check stop_reason; ⚠ assistant-prefill removed on 4.6+ (400) → Structured Outputs output_config.format. Explain-back passed. BUILD BANKED: worksheet 16/16 green — build_extraction_call assembles a correct current-API extraction payload w/ schema-enforced structured output. Ready for /sharpen.)*
- [~] **Zero-shot, few-shot, role prompting** — steering by examples and persona; clear-and-direct. `T1` · src: Eden Marco lec 69–70, Anthropic *(2026-07-20 — taught as the 2026 steering ladder: zero-shot+why-behind-rules default → few-shot (3–5, `<examples>` tags, DIVERSE) for fuzzy steering → structured outputs for guaranteed shape (shape ≠ quality); role = prior-shift in system param (cacheable prefix), changes tone+scope/judgment, NOT knowledge; ⚠ "CRITICAL: You MUST" now overtriggers on 4.5/4.6+. Explain-back passed. Build pending: worksheet 2026-07-20_zero-few-shot-role-prompting.py)*
...
- [2026-07-20](reflection_log/2026-07-20.md) — §1.3 "Zero-shot, few-shot, role prompting" (Tier 1): the 2026 steering ladder (instructions → examples → schema), few-shot spec (3–5, tagged, diverse), role as cacheable-prefix prior-shift (tone+judgment, not knowledge), ⚠ aggressive prompting now overtriggers; old tutorial pages consolidated into one model-versioned best-practices page. Explain-back passed; build pending (triage-call worksheet).
