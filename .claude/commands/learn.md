...
There are **four curated curricula + one on-demand mode**. Resolve which one this session is for *before reading anything else*:

| Token in the command | Track | Roadmap (source of `[ ]` units) | Session log | Worksheet path + naming |
|---|---|---|---|---|
| `api`, `apisec`, `interview` | **Secure API Engineering on AWS** *(⏱ TIME-BOXED — interview 2026-08-17. Outranks all other tracks until then, then goes dormant.)* | `career/api-security-aws-prep/summary.md` | `career/api-security-aws-prep/reflection_log/YYYY-MM-DD.md` | `career/lesson_assignments/apisec-<section>-<kebab-unit>.py` (e.g. `apisec-4.4-jwt-validation-checklist.py`) |
