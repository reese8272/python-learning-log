...
4. **PART 3 — project (rung 5, the Step 7 build)**: the bank-gate build, written as a scaffolded stub with the same struggle-first bar. This is where the drilled facets come together into the thing that banks the unit. Where the track has a real project repo, **PART 3 *is* that section's issue** (for `api`: the matching issue in `~/workspace/secure-api-lab/docs/issues.md`).
