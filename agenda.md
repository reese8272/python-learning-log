...
>
> **The two calls: (1) ✅ `/drill` — DONE 08-18 (run early, not 8:30).** Sockets first, and it cleared on the 3rd ask. The rotation is no longer at zero. **(2) `/learn` §1.2b at peak tomorrow** — still owed, and the **`scratch_blocking.py` build is owed first** (5 min, laptop; it was blocked tonight by being on a phone).
>
...
**Stopped at:** 08-18 (evening) — **`/drill` ran, the rotation is open.** 2 items, ~10 min, phone. ① **sockets cleared on the 3rd ask** (under a probe, not cold — re-ask in ~1 week). ② **pre-hook vs post-hook missed cold** — re-taught in plain language, **explain-back not completed** (closed on low energy, correctly). **Next drill opens with the pre/post-hook explain-back.** Earlier today: **`/learn api` §1 chunk 1.2a. Unit 1.2 (`async def` vs `def` + the threadpool trap) is `[~]`, not banked.** The 6-day stall (08-12 → 08-17) after the rejection was the all-or-nothing signature again; it ended today with a full peak-window rep. Pretest went **0 for 3** — he predicted `async def` beats plain `def` on a blocking call, and it's **25× worse** (~100s vs ~4s). Session: `career/secure-api-engineering/reflection_log/2026-08-18.md`.
